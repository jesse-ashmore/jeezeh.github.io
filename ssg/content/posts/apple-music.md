...
I started with trying to import my music from Windows, as my MacBook Air doesn't have enough storage to hold all my ripped physical media at once.

On Windows, Apple Music uses your local user's _Music_ folder to store your library data, cache, downloaded tracks, and imported music (if you choose to copy imported files). There is a handy option in the settings which allows you to change this:

<!-- Screenshot -->

**This setting does not (entirely) work.** While some dimensions of Apple Music data respects this location, the Apple Music Windows App still stores some data in the default location (your user's _Music_ folder). This is a problem if you backup your Music folder to something like OneDrive, which many users will do. The Apple Music library data consists of a few DB files, and a number of folders and files _for every song you playlist/store in your library_ (e.g. metadata and artwork). This quickly adds up to a huge number of files trying to sync and subsequently being locked by Apple Music app during the process.

{% alert(tip=true) %}
It's possible that you could manipulate the default location through editing some preference or registry files, but I decided to bite the bullet and **remove the _Music_ from OneDrive backup management**, instead moving the folder to OneDrive directly and leaving my 'official' user's Music folder empty.

This is one of many behavioural changes that Apple Music will ask of its users that do not follow the yellow brick road to Cupertino.
{% end %}
...
I never found the cause of the first issue. Unfortunately, it seems with a large enough set of tracks to upload (from Windows at least), the app will lose track (hah!) of certain songs during the upload/matching process and leave you with partial albums.

{% alert(tip=true) %}
The fix for missing songs in albums was to retry the upload from a totally separate device. It's possible that wiping my local library state and trying to import again could have worked, but numerous attempts at deleting everything from my Library and re-uploading left the same songs missing from albums after multiple attempts.

The only thing that worked was to try and re-upload all my albums again from a Mac, which seemed to fill in the missing gaps.
{% end %}

The issue with missing playlist songs is more insidious.

## Respect the Library

You must respect the Library. Coming from Spotify, it's easy to assume the library is just an arbitrary list of songs you might have "Liked" or added to playlists over the years. In fact, in Apple Music, this little setting makes the behavior feel identical:

![]()

But beware! The Library is not just some arbitrary list, but may be the source of truth for all your music (should you choose to use it). Songs can be added to and removed from playlists without altering your library, **but changes to your Library will affect your playlists!**. From its iTunes roots, this makes more sense in hindsight, but is not something I considered at first.

**If you delete a song from your library, the delete will propagate to any playlists the song is a part of too.**

I had enabled the _auto-add to library_ feature without much thought, but later decided that I wanted to try and keep my Library and playlists separate, with the former used only for 'owned' music. I decided to delete a bunch of songs from my library which were added when updating my playlists, and with it, my playlists were affected too.

### Leaning into the Library

Rather than trying to use the library as a separate storage to my playlists, I've decided to lean in and use the Library to track all music that I want to keep around in some form. On desktop, I can use the powerful Library filters to narrow down lists of albums/artists/songs to just those I've uploaded/own if I wish, and on mobile I can use album searches and smart playlists for more targeted listening.
...
For whatever reason, it takes a second or so for each song to be added to the library, meaning that I needed to leave my Mac on overnight to process all additions.

Plugged in, I ran `caffeinate -s -i -u` to keep it awake, and left Apple Music in focus to work away.
{% end %}

**Now that I have my full Library imported, it's time to listen to some music**.

## The listening experience

### On Windows

- Nvidia Fairplay vs. Widevine
